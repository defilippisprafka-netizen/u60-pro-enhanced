...
static int helper(const char *command){pid_t p=fork();if(!p){int n=open("/dev/null",O_RDWR);dup2(n,0);dup2(n,1);dup2(n,2);if(n>2)close(n);execl("/data/u60-panel/wifi-relay.sh","wifi-relay.sh",command,(char*)NULL);_exit(127);}int s=0;if(p<0)return 0;while(waitpid(p,&s,0)<0)if(errno!=EINTR)return 0;return WIFEXITED(s)&&!WEXITSTATUS(s);}
...
static cJSON *scan(void){if(access(RUN "/enabled",F_OK)==0)return result(0,"请先断开中继再扫描");if(!helper("prepare"))return result(0,"请先开启5G热点、关闭2.4G/访客热点，并将USB网口设为LAN");if(!ack("SCAN")){helper("scan-stop");return result(0,"无线扫描繁忙，请稍后重试");}char b[32768];cJSON*net=NULL;for(int n=0;n<8;n++){sleep(1);if(ctrl("SCAN_RESULTS",b,sizeof(b))){cJSON_Delete(net);net=parse_scan(b);if(cJSON_GetArraySize(net)>0)break;}}helper("scan-stop");if(!net)return result(0,"扫描结果读取失败");cJSON*j=result(1,"请选择上游Wi-Fi");cJSON_AddItemToObject(j,"networks",net);return j;}
static cJSON *connect_ap(const cJSON*a){
 ...
 if(access(RUN "/enabled",F_OK)==0)return result(0,"请先断开当前中继，再切换网络");
 if(!helper("prepare"))return result(0,"请先开启5G热点、关闭2.4G/访客热点，并将USB网口设为LAN");
 ...
 for(int n=0;n<30;n++){char state[64];if(ctrl("STATUS",b,sizeof(b))&&keyval(b,"wpa_state",state,sizeof(state))&&!strcmp(state,"COMPLETED")){if(!ack("SAVE_CONFIG")||!helper("enable"))goto fail;return result(1,"已连接并保存，正在获取上游地址；以中继状态为准");}sleep(1);}
 fail:memset(cmd,0,sizeof(cmd));if(id>=0){snprintf(cmd,sizeof(cmd),"REMOVE_NETWORK %d",id);ack(cmd);}helper("off");return result(0,"中继未连接，已保留原出口；请核对密码、信号及热点并发状态");
}
...
int main(int argc,char**argv){umask(077);struct rlimit lim={0,0};setrlimit(RLIMIT_CORE,&lim);if(argc!=2)return 2;cJSON*j=NULL,*a=NULL;char b[8192]={0};if(!strcmp(argv[1],"connect")||!strcmp(argv[1],"lease")){size_t n=fread(b,1,sizeof(b)-1,stdin);const char*end=NULL;if(n==sizeof(b)-1||!(a=cJSON_ParseWithOpts(b,&end,1))||!cJSON_IsObject(a))j=result(0,"无效请求");}
 if(!j){if(!strcmp(argv[1],"scan"))j=scan();else if(!strcmp(argv[1],"connect"))j=connect_ap(a);else if(!strcmp(argv[1],"lease"))j=lease(a);else j=result(0,"未知操作");}char*out=cJSON_PrintUnformatted(j);puts(out?out:"{}");free(out);memset(b,0,sizeof(b));cJSON_Delete(a);cJSON_Delete(j);return 0;}
#endif
