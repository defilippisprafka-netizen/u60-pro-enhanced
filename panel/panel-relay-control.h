...
static cJSON *relay_run(const char*command,const cJSON*args){
 ...
 return reply(ok,ok?(!strcmp(command,"off")?"中继已断开，恢复原出口；已保存的网络仍保留":"正在连接已保存的上游"):"操作未完成，请确认USB为LAN、仅开启5G热点且已保存网络");
}
static cJSON *relay_action(const char*action,const cJSON*args){
 ...
 if(!strcmp(action,"wifi.relay.select")){
  ...
  cJSON*i=item(s,"relay-connect",ssid,"form","输入密码","wifi.relay.connect",1,"连接并在U60保存上游凭据，重启自动重连；断线可能回退蜂窝。中继期间仅支持5G热点，IPv6互联网暂停。");
  cJSON_AddStringToObject(jget(i,"args"),"ssid",ssid);cJSON_AddStringToObject(jget(i,"args"),"security",security);cJSON_AddStringToObject(jget(i,"args"),"bssid",bssid);
 ...
 if(!strcmp(action,"wifi.relay.scan")){
  ...
  cJSON*n;cJSON_ArrayForEach(n,jget(r,"networks")){if(strcmp(jstr(n,"security"),"unsupported")==0)continue;cJSON*c=cJSON_CreateObject();char label[128];snprintf(label,sizeof(label),"%s · %s",jstr(n,"ssid"),jget(n,"frequency")&&jget(n,"frequency")->valueint<3000?"2.4G":"5G");cJSON_AddStringToObject(c,"label",label);char desc[64];snprintf(desc,sizeof(desc),"%s · %.0f dBm",jstr(n,"security"),jget(n,"signal")?jget(n,"signal")->valuedouble:0);cJSON_AddStringToObject(c,"description",desc);cJSON*a=cJSON_AddObjectToObject(c,"args");for(int k=0;k<3;k++){const char*key=(const char*[]){"ssid","bssid","security"}[k];cJSON_AddStringToObject(a,key,jstr(n,key));}cJSON_AddItemToArray(choices,c);}
...
static void relay_items(cJSON*s,cJSON*data){
 ...
 cJSON*i=item(s,"relay","Wi-Fi 中继",enabled||saved?"action":"info",text,enabled?"wifi.relay.off":saved?"wifi.relay.on":"wifi.relay.scan",known,"中继保留5G热点；掉线回退原出口，可能消耗蜂窝流量。开机自动重连已保存网络。");cJSON_ReplaceItemInObject(i,"confirm",cJSON_CreateBool(enabled||saved));
 if(!enabled)item(s,"relay-scan",saved?"连接其他 Wi-Fi":"连接上游 Wi-Fi","action","扫描附近网络","wifi.relay.scan",known,"请关闭2.4G/访客热点，USB设为LAN");
 if(active)cJSON_ReplaceItemInObject(data,"wifi_status",cJSON_CreateString("中继上网 · 5G热点"));
